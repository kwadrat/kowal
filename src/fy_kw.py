...
"""
lxa_21_inst = """\
UPDATE uu_power SET m_samples='{NULL,0,0,1.5,2.5,3}' WHERE k_sample=8 AND f_object=123 AND m_date='2013-01-31';"""
lxa_22_inst = """\
...
lxa_38_inst = """\
SELECT m_date, m_samples, m_none, m_zero, m_sum FROM uu_energy WHERE f_object=7 ORDER BY m_date;"""
lxa_39_inst = """\
SELECT %(e_date)s, %(e_samples)s, %(e_none)s, %(e_zero)s, %(e_sum)s FROM %(table_name)s WHERE %(e_object)s=%(f_object)d ORDER BY %(e_date)s;"""
