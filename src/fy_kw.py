...
lxa_23_inst = """\
SELECT %(part_my_fields)s FROM %(my_table_name)s WHERE %(part_my_limits)s ORDER BY m_date;"""
lxa_24_inst = """\
...
lxa_39_inst = """\
SELECT %(part_my_fields)s FROM %(my_table_name)s%(wstawka_war)s ORDER BY %(e_date)s;"""
