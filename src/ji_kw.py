...
class EnergyReader(CommonReader):
    ...
    def detect_sheet_header(self):
        ...
        if got_val == u'w kWh':
            ...
            got_val = self.vx_peek('M', 1 + self.tel_delta)
            if got_val == u'w kWh':
                pass
            else:
    ...
    def fetch_energy_field(self, key_object, single_row, row_date, spring_dst_date, autumn_dst_date, sample_index):
        '''
        EnergyReader:
        '''
...
class AugmentedEnReader(EnergyReader):
    ...
    def fill_a_case(self):
        ...
        self.vx_poke('L', 2, u'zmieniony_a')
